<template>
  ...
  <div class="bg-white flex-grow overflow-auto">
    <div class="w-full h-full text-sm">
      <div v-if="state.work_panel == 'PROJECT' && project.root.value">
        <json-viewer :value="project" expanded copyable :expand-depth="10" show-double-quotes show-array-index>
        </json-viewer>
      </div>
  ...
  <div v-if="project.root.value" class="bg-1 h-10 flex-none w-full border-t border-gray-300 flex items-center p-1">
    <button @click.stop="saveProject()" type="button"
      ...
      <div class="flex items-center">
        ...
        </svg><span class="ml-1">Save project</span>
      </div>
    </button>
    <button @click.stop="deleteProjectCheck()" type="button"
      ...
      <div class="flex items-center">
        ...
        </svg><span class="ml-1">Delete project</span>
      </div>
    </button>
    <ModalBlank id="danger-modal" :modalOpen="deleteProjectModal" @close-modal="deleteProjectModal = false">
      <div class="p-5 flex space-x-4">
        <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-red-100">
          <svg class="w-4 h-4 shrink-0 fill-current text-red-600" viewBox="0 0 16 16">
            <path
...
const saveProject = () => {
  ...
  form.post('/project/save', {
    onSuccess: () => {
      project.value.data.updated_at = updated_at
      notify({
        type: 'success',
        title: 'Message',
